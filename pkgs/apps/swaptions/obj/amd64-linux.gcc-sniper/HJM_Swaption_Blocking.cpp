  ...
  
{
  ...
  int* perf_vec = perf_vector(lTrials/BLOCKSIZE, pr);
  int pr_i = 0;
  for (l=0;l<=lTrials-1;l+=BLOCKSIZE) {
      if(!perf_vec[pr_i]){ pr_i++; continue; }
      pr_i++;
  ...
  dSimSwaptionMeanPrice = (dSumSimSwaptionPrice * get_mf(lTrials/BLOCKSIZE, pr)) / lTrials;
  // dSimSwaptionMeanPrice = dSumSimSwaptionPrice / lTrials;

  dSimSwaptionStdError = sqrt(((dSumSquareSimSwaptionPrice * get_mf(lTrials/BLOCKSIZE, pr)) -(dSumSimSwaptionPrice * get_mf(lTrials/BLOCKSIZE, pr)) *(dSumSimSwaptionPrice * get_mf(lTrials/BLOCKSIZE, pr)) /lTrials)/

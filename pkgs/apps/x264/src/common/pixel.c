...
#include "perforation/perforation.h"
...
{                                                   \
    int i_sum = 0;                                  \
    int x, y;                                       \
    int pr0 = get_loop_rate(0);                     \
    int pr1 = get_loop_rate(1);                     \
                                                    \
    int* pr_y_vec = perf_vector(ly, pr0);           \
    int* pr_x_vec = perf_vector(lx, pr1);           \
    for( y = 0; y < ly; y++ )                       \
    {                                               \
        if(!pr_y_vec[y]) continue;                  \
        for( x = 0; x < lx; x++ )                   \
        {                                           \
            if(!pr_x_vec[x]) continue;              \
            i_sum += abs( pix1[x] - pix2[x] );      \
        }                                           \
        pix1 += i_stride_pix1;                      \
        pix2 += i_stride_pix2;                      \
    }                                               \
    float mf_y = get_mf(ly, pr0);                   \
    float mf_x = get_mf(lx, pr1);                   \
    free_vec(pr_y_vec);                             \
    free_vec(pr_x_vec);                             \
    return (int) ((float) i_sum * mf_y * mf_x);     \
}
...
static int pixel_satd_wxh( uint8_t *pix1, int i_pix1, uint8_t *pix2, int i_pix2, int i_width, int i_height )
{
    ...
    int pr0 = get_loop_rate(2);
    int pr1 = get_loop_rate(3);
    
    int* pr_y_vec = perf_vector(i_height/4, pr0);
    int* pr_x_vec = perf_vector(i_width/4, pr1);
    int i_pr_y = 0;
    int i_pr_x = 0;
    for( y = 0; y < i_height; y += 4)
    {
        if(!pr_y_vec[i_pr_y]) { i_pr_y++; continue; }
        i_pr_y++;
        i_pr_x = 0;
        for( x = 0; x < i_width; x += 4)
        {
            if(!pr_x_vec[i_pr_x]) { i_pr_x++; continue; }
            i_pr_x++;
    ...
    return (int) ((float)i_satd * get_mf(i_width, pr1) * get_mf(i_height, pr0)) / 2;
}

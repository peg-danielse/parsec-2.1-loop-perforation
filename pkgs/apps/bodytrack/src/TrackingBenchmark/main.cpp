...
bool ProcessCmdLine(int argc, char **argv, string &path, int &cameras, int &frames, int &particles, int &layers, int &threads, int &threadModel, bool &OutputBMP, string &outpath)
{
	...
	if(argc > 8){
		outpath = string(argv[8]);														//get output path and add backslash if needed
		if(outpath[path.size() - 1] != DIR_SEPARATOR[0])
			outpath.push_back(DIR_SEPARATOR[0]);
	}
...
#if defined(USE_OPENMP)
int mainOMP(string path, int cameras, int frames, int particles, int layers, int threads, bool OutputBMP, string outpath)
{
	...
	if (enable_heartbeats == 1) {
		heartbeat(heart, frames); // Beat indicating work end
	}
...
int mainPthreads(string path, int cameras, int frames, int particles, int layers, int threads, bool OutputBMP, string outpath)
{
	...
	cout << "Using dataset : " << path << endl;
	cout << particles << " particles with " << layers << " annealing layers" << endl << endl;
	ofstream outputFileAvg((outpath + "poses.txt").c_str());

	cout << "logging output to: " << (outpath + "poses.txt").c_str() << endl;
	...
	if (enable_heartbeats == 1  && tid == 0)
	{
		heartbeat(heart, frames);
	}
#if defined(ENABLE_PARSEC_HOOKS)
        __parsec_roi_end();
#endif
...
#define __PARSEC_XSTRING(x) __PARSEC_STRING(x)
	...
	if(!ProcessCmdLine(argc, argv, path, cameras, frames, particles, layers, threads, threadModel, OutputBMP, outpath))	
		return 0;
	...
	switch(threadModel)
	{
		case 0 : 
			...
			break;
                ...
                case 2 :
                        #if defined(USE_THREADS)
                                mainPthreads(path, cameras, frames, particles, layers, threads, OutputBMP, outpath);             //Posix threads tracking
                                break;
                        #else
